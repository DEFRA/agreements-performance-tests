...
# ============================================
# FRPS test — report goes to root reports dir
# so CDP portal finds index.html at the top level
# ============================================
FRPS_SCENARIO=test
...
jmeter -n -t ${FRPS_SCENARIOFILE} -e -l "${FRPS_REPORTFILE}" -o ${JM_REPORTS} -j ${FRPS_LOGFILE} -f -Jenv="${ENVIRONMENT}" -Jcsv_path="${JM_DATA}" -Juser_count="${USER_COUNT}" -Jramp_up_period_seconds="${RAMP_UP_PERIOD_SECONDS}" -Jduration_seconds="${DURATION_SECONDS}"
FRPS_EXIT=$?
echo "FRPS test exit code: ${FRPS_EXIT}"

# ============================================
# WMP test — report goes to wmp/ subdirectory
# ============================================
...
if [ -n "$RESULTS_OUTPUT_S3_PATH" ]; then
  if [ -f "${JM_REPORTS}/index.html" ] || [ -f "${WMP_REPORTS}/index.html" ]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      echo "  FRPS report: $RESULTS_OUTPUT_S3_PATH/index.html"
